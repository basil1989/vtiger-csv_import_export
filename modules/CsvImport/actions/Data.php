...
class CsvImport_Data_Action extends Vtiger_Action_Controller {
	...
	public function initializeImport() {
		...
		if ($lockInfo != null) {
			if($lockInfo['userid'] != $this->user->id) {
				CsvImport_Utils_Helper::showImportLockedError($lockInfo);
				return false;
			} else {
				return true;
			}
		} else {
			CsvImport_Lock_Action::lock($this->id, $this->module, $this->user);
			return true;
		}
	}

	public function finishImport() {
		CsvImport_Lock_Action::unLock($this->user, $this->module);
		CsvImport_Queue_Action::remove($this->id);
	}
	...
	public function updateImportStatus($entryId, $entityInfo) {
		...
		$adb->pquery('UPDATE ' . CsvImport_Utils_Helper::getDbTableName($this->user) . ' SET status=?, recordid=? WHERE id=?',
	...
	public function createRecords() {
        ...
        $entityData = array();
		$tableName = CsvImport_Utils_Helper::getDbTableName($this->user);
		$sql = 'SELECT * FROM ' . $tableName . ' WHERE status = '. CsvImport_Data_Action::$IMPORT_RECORD_NONE;
		...
		for ($i = 0; $i < $numberOfRecords; ++$i) {
			...
			if(method_exists($focus, 'importRecord')) {
				...
				if (!empty($mergeType) && $mergeType != CsvImport_Utils_Helper::$AUTO_MERGE_NONE) {
                    ...
                    if ($noOfDuplicates > 0) {
						if ($mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_IGNORE) {
							$entityInfo['status'] = self::$IMPORT_RECORD_SKIPPED;
						} elseif ($mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_OVERWRITE ||
								$mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_MERGEFIELDS) {
							...
							if ($mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_OVERWRITE) {
							...
							if ($mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_MERGEFIELDS) {
            ...
            if($createRecord || $mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_MERGEFIELDS || $mergeType == CsvImport_Utils_Helper::$AUTO_MERGE_OVERWRITE){
                $entityIdComponents = vtws_getIdComponents($entityInfo['id']);
                $recordId = $entityIdComponents[1];
	...
	public function transformForImport($fieldData, $moduleMeta, $fillDefault=true, $checkMandatoryFieldValues=true) {
		...
		foreach ($fieldData as $fieldName => $fieldValue) {
			...
			if ($fieldInstance->getFieldDataType() == 'owner') {
				...
				if(empty($ownerId) ||
							!CsvImport_Utils_Helper::hasAssignPrivilege($moduleMeta->getEntityName(), $ownerId)) {
				...
				if (!empty($fieldValue)) {
						...
						foreach ($referencedModules as $referenceModule) {
							...
							if ($referenceModule == 'Users') {
								...
								if(empty($referenceEntityId) ||
										!CsvImport_Utils_Helper::hasAssignPrivilege($moduleMeta->getEntityName(), $referenceEntityId)) {
					...
					if ($referencedModules[0] == 'Users') {
						...
						if(empty($fieldData[$fieldName]) ||
								!CsvImport_Utils_Helper::hasAssignPrivilege($moduleMeta->getEntityName(), $fieldData[$fieldName])) {
	...
	public static function getScheduledImport() {
		...
		$importQueue = CsvImport_Queue_Action::getAll(CsvImport_Queue_Action::$IMPORT_STATUS_SCHEDULED);
		foreach($importQueue as $importId => $importInfo) {
			...
			$scheduledImports[$importId] = new CsvImport_Data_Action($importInfo, $user);
		}
    ...
    public static function getImportDetails($user){
        $adb = PearDatabase::getInstance();
        $tableName = CsvImport_Utils_Helper::getDbTableName($user);
