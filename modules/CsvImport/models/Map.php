...
class CsvImport_Map_Model extends Vtiger_Base_Model {
	...
	public static function getInstanceFromDb($row, $user) {
		...
		return new CsvImport_Map_Model($map, $user);
	}
	...
	public static function getAllByModule($moduleName) {
		...
		for($i=0; $i<$noOfMaps; ++$i) {
			$importMap = CsvImport_Map_Model::getInstanceFromDb($db->query_result_rowdata($result, $i), $current_user);
