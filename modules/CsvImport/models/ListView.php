 ...
 */
class CsvImport_ListView_Model extends Vtiger_ListView_Model {
	...
	public function getLastImportedRecord() {
		...
		$userDBTableName = CsvImport_Utils_Helper::getDbTableName($user);

		$result = $db->pquery('SELECT recordid FROM '.$userDBTableName.' WHERE status NOT IN (?,?) AND recordid IS NOT NULL',Array(CsvImport_Data_Action::$IMPORT_RECORD_FAILED,  CsvImport_Data_Action::$IMPORT_RECORD_SKIPPED));
