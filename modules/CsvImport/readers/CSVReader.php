...
class CsvImport_CSVReader_Reader extends CsvImport_FileReader_Reader {
	...
	public function getColumnData($hasHeader, $columnIdx) {
		global $default_charset;
		...
		while($data = fgetcsv($fileHandler, 0, ',')) {
			if ($currentRow > 0){
				...
				foreach($data as $key => $value) {
					if ($i + 1 == $columnIdx){
						$columnData[$currentRow - 1] = $this->convertCharacterEncoding($value, $this->request->get('file_encoding'), $default_charset);
					}
					$i++;
				}
			}
