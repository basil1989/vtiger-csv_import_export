...
class CsvImport_Main_View extends Vtiger_View_Controller{
	...
	public static function import($request, $user) {
		...
		$importController = new CsvImport_Main_View($request, $user);
		...
		// 	$importInfo = CsvImport_Queue_Action::getUserCurrentImportInfo($importController->user);
	...
	public function triggerImport($batchImport=false) {
		$importInfo = CsvImport_Queue_Action::getImportInfo($this->request->get('module'), $this->user);
		$importDataController = new CsvImport_Data_Action($importInfo, $this->user);

		if(!$batchImport) {
			if(!$importDataController->initializeImport()) {
				CsvImport_Utils_Helper::showErrorPage(vtranslate('ERR_FAILED_TO_LOCK_MODULE', 'Import'));
				exit;
			}
		}

		$importDataController->importData();
		CsvImport_Queue_Action::updateStatus($importInfo['id'], CsvImport_Queue_Action::$IMPORT_STATUS_HALTED);
		$importInfo = CsvImport_Queue_Action::getImportInfo($this->request->get('module'), $this->user);
	...
	public static function showImportStatus($importInfo, $user) {
		if($importInfo == null) {
			CsvImport_Utils_Helper::showErrorPage(vtranslate('ERR_IMPORT_INTERRUPTED', 'Import'));
			exit;
		}
		$importDataController = new CsvImport_Data_Action($importInfo, $user);
		if($importInfo['status'] == CsvImport_Queue_Action::$IMPORT_STATUS_HALTED ||
				$importInfo['status'] == CsvImport_Queue_Action::$IMPORT_STATUS_NONE) {
			$continueImport = true;
		} else {
...
//			}
	...
	public function saveMap() {
		...
		if($saveMap && !empty($mapName)) {
			...
			$fileReader = CsvImport_Utils_Helper::getFileReader($this->request, $this->user);
			if($fileReader == null) {
				return false;
			}
			...
			$importMap = new CsvImport_Map_Model($map, $this->user);
			$importMap->save();
		}
	}
	...
	public function copyFromFileToDB() {
		...
		if($fileReader->getStatus() == 'success') {
			...
			CsvImport_Utils_Helper::showErrorPage(vtranslate('ERR_FILE_READ_FAILED', 'Import').' - '.
	...
	public function queueDataImport() {
		...
		CsvImport_Queue_Action::add($this->request, $this->user);
	}
	...
	public static function deleteMap($request) {
		...
		if(!empty($mapId)) {
			CsvImport_Map_Model::markAsDeleted($mapId);
		}
